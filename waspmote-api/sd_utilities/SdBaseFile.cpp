						...
						uint8_t oflag) 						
{
	...
	{		
		...
		for (index = 0; index < 16; index++) 
		{
			...
			if( (p->name[0] == DIR_NAME_FREE || p->name[0] == DIR_NAME_DELETED ) && strcmp((char*)dname,"..")!=0 ) 
			{
...
int8_t SdBaseFile::lsPrintNext(uint8_t flags, uint8_t indent) 
{
  ...
  for (uint8_t i = 0; i < indent; i++) USB.print(' ');
