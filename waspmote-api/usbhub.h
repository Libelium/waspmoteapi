...
class USBHub :private USBDeviceConfig 
{
	static bool bResetInitiated;				// True when reset is triggered

	USBDEV		*pUsb;					// USB class instance pointer
...
void PrintHubPortStatus(USBDEV *usbptr, uint8_t addr, uint8_t port, bool print_changes = false);
